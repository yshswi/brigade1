...
const data = [
  { rank: 1, name: "Kodathi",      scaled: 93, timelines: 2, template: 3, governance: 3, accOnboard: 3, accUtil: 3,   rationale: "Timeline delay of ~5 weeks" },
  { rank: 2, name: "ArcotRoad",    scaled: 92, timelines: 3, template: 3, governance: 3, accOnboard: 2, accUtil: null, rationale: "Historical files not uploaded" },
  { rank: 3, name: "Neopolis_2",   scaled: 92, timelines: 2, template: 3, governance: 3, accOnboard: 3, accUtil: null, rationale: "Timeline delay of 1 week" },
  { rank: 4, name: "Banashankari", scaled: 83, timelines: 3, template: null, governance: null, accOnboard: 2, accUtil: null, rationale: "Historical files not uploaded" },
  { rank: 5, name: "ChiranFort",   scaled: 83, timelines: 1, template: 3, governance: 3, accOnboard: 3, accUtil: null, rationale: "Timeline delay of 8 weeks" },
  { rank: 6, name: "Vanagaram2",   scaled: 80, timelines: 3, template: 3, governance: 3, accOnboard: 2, accUtil: 1,   rationale: "1 workflow missed (Arch. CD signoff); historical files not uploaded" },
  { rank: 7, name: "Kompally",     scaled: 67, timelines: 1, template: null, governance: 3, accOnboard: 2, accUtil: null, rationale: "Delay of 17 weeks; historical files not uploaded" },
];
...
function ScoreDots({ value, justify = "center" }) {
  ...
  if (value === null) return <span style={{ color:"#CBD5E0", fontSize:11 }}>—</span>;
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ display:"flex", gap: hovered ? 4 : 3, justifyContent: justify,
        transform: hovered ? "scale(1.3)" : "scale(1)", transition:"transform 0.25s ease, gap 0.25s ease", cursor:"default" }}>
      {[1,2,3].map(i => (
        <div key={i} style={{ width:8, height:8, borderRadius:"50%",
          background: i <= value ? color : "#E2E8F0",
          boxShadow: hovered && i <= value ? glow : "none", transition:"box-shadow 0.25s ease" }}/>
      ))}
    </div>
  );
}
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ display:"flex", flexDirection:"column", alignItems:"center",
        transform: hovered ? "scale(1.12)" : "scale(1)", transition:"transform 0.25s ease", cursor:"default" }}>
      <span style={{ fontSize: small ? 13 : 18, fontWeight:800,
        color: hovered ? BRIGADE_DARK : BRIGADE_BLUE, fontVariantNumeric:"tabular-nums", transition:"color 0.25s ease" }}>
        {value}%
      </span>
      {!small && (
        <div style={{ width:"2.4rem", height:3, background:"#EDF2F7", borderRadius:2, marginTop:3, overflow:"hidden", position:"relative" }}>
          <div style={{ height:"100%", borderRadius:2, width:`${value}%`, background:`linear-gradient(90deg,${BRIGADE_DARK},${BRIGADE_BLUE})` }}/>
          {hovered && <div style={{ position:"absolute", inset:0, background:"linear-gradient(90deg,transparent,rgba(255,255,255,0.75),transparent)", animation:"barShimmer 0.7s ease forwards" }}/>}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{ position:"relative", display:"inline-block", width: emojiSize * 2.2, height: emojiSize * 2.2 }}>
      <div style={{ position:"absolute", top:"50%", left:"50%", transform:"translate(-50%,-50%)",
        width: emojiSize * 2, height: emojiSize * 2, borderRadius:"50%",
        background:`radial-gradient(circle, ${accent}45 0%, ${accent}12 45%, transparent 70%)`,
        animation:`medalShine 2.8s ease-in-out infinite`, animationDelay:`${animDelay}s`,
      ...
      {[
        ...
        <div key={i} style={{
          ...
          width: b.w, height: b.h, borderRadius: emojiSize,
          background: b.h > b.w
      ...
      <span style={{ position:"absolute", top:"50%", left:"50%",
        transform:"translate(-50%,-50%)", fontSize: emojiSize, display:"block",
        zIndex:3, animation:`medalFloat 3s ease-in-out infinite`, animationDelay:`${animDelay}s` }}>
        {emoji}
      </span>
    </div>
  );
...
function BorderGlimmer({ medalIndex, height }) {
  ...
  return (
    <div style={{ position:"absolute", top:0, left:0, right:0, height,
      borderRadius:"14px 14px 0 0", overflow:"hidden", background: m.shimmerBase }}>
...
function PodiumCardDesktop({ p, podiumPos, delay }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", justifyContent:"flex-end", flex: isGold ? 1.15 : 1 }}>
      <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
        style={{ background:"#FFFFFF", border:`1.5px solid ${m.border}`, borderRadius:14,
          padding: cardPad[podiumPos], position:"relative", overflow:"hidden",
          opacity: vis ? 1 : 0,
        ...
        {hovered && <div style={{ position:"absolute", inset:0, pointerEvents:"none", zIndex:3,
        ...
        <div style={{ position:"relative", zIndex:2 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: isGold ? 14 : 10 }}>
            <MedalWithGlint emoji={m.emoji} accent={m.accent} glintColor={m.glintColor} emojiSize={emojiSizes[podiumPos]} animDelay={podiumPos * 0.9}/>
            <span style={{ background:m.light, color:m.text, border:`1px solid ${m.border}`, borderRadius:20, padding:"3px 10px", fontSize:9, fontWeight:700, letterSpacing:2, textTransform:"uppercase" }}>{m.label}</span>
          </div>
          <div style={{ fontSize:nameSizes[podiumPos], fontWeight:800, color:"#0D1F35", marginBottom:6, letterSpacing:-0.3, lineHeight:1.1 }}>{p.name}</div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px 10px", marginBottom:12 }}>
            {metricKeys.map((key, i) => (
              <div key={key}>
                <div style={{ fontSize:8.5, color:"#A0AEC0", fontWeight:700, letterSpacing:1, textTransform:"uppercase", marginBottom:4, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{metricLabels[i]}</div>
          ...
          <div style={{ fontSize:10, color:"#718096", lineHeight:1.6, fontStyle:"italic" }}>
            <span style={{ fontWeight:700, color:"#A0AEC0", fontStyle:"normal", letterSpacing:1, textTransform:"uppercase", fontSize:8.5 }}>Rationale </span>
            {p.rationale}
          </div>
        </div>
      </div>
      <div style={{ height: podiumBaseHeights[podiumPos], background:`linear-gradient(180deg, ${m.light} 0%, ${m.border}55 100%)`, border:`1px solid ${m.border}`, borderTop:"none", borderRadius:"0 0 6px 6px" }}/>
    </div>
  );
}
...
function PodiumCardMobile({ p, delay }) {
  ...
  return (
    <div style={{ background:"#FFFFFF", border:`1.5px solid ${m.border}`, borderRadius:14,
      padding:"16px 18px", position:"relative", overflow:"hidden",
      opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(20px)",
      transition:"opacity 0.5s ease, transform 0.5s ease", boxShadow:`0 4px 20px ${m.accent}22` }}>
      <div style={{ position:"absolute", top:0, left:0, right:0, height:3, borderRadius:"14px 14px 0 0", overflow:"hidden", background:m.shimmerBase }}>
        <div style={{ position:"absolute", inset:0,
          background:`linear-gradient(90deg, transparent 0%, white 45%, white 55%, transparent 100%)`,
          backgroundSize:"200% 100%", animation:`borderGlimmer 6s ease-in-out infinite`, animationDelay:`${mobileAnimDelay}s` }}/>
      </div>
      <div style={{ display:"flex", alignItems:"flex-start", gap:16 }}>
        <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-start", minWidth:110 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6, marginTop:4 }}>
            <MedalWithGlint emoji={m.emoji} accent={m.accent} glintColor={m.glintColor} emojiSize={32} animDelay={(p.rank-1)*0.9}/>
            <span style={{ background:m.light, color:m.text, border:`1px solid ${m.border}`, borderRadius:20, padding:"2px 9px", fontSize:9, fontWeight:700, letterSpacing:2, textTransform:"uppercase" }}>{m.label}</span>
          </div>
          <div style={{ fontSize:20, fontWeight:800, color:"#0D1F35", letterSpacing:-0.3, lineHeight:1.1, marginBottom:4 }}>{p.name}</div>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"8px 12px", marginBottom:10 }}>
            {metricKeys.map((key, i) => (
              <div key={key}>
                <div style={{ fontSize:8, color:"#A0AEC0", fontWeight:700, letterSpacing:1, textTransform:"uppercase", marginBottom:4, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{metricLabels[i]}</div>
                <ScoreDots value={p[key]} justify="flex-start"/>
              </div>
            ))}
          </div>
          ...
          <div style={{ fontSize:10, color:"#718096", lineHeight:1.6, fontStyle:"italic" }}>
            <span style={{ fontWeight:700, color:"#A0AEC0", fontStyle:"normal", letterSpacing:1, textTransform:"uppercase", fontSize:8 }}>Rationale </span>
            {p.rationale}
          </div>
        </div>
      </div>
...
export default function App() {
  ...
  useEffect(() => { rest.forEach((_,i) => setTimeout(() => setRowVis(v=>[...v,i]), 1000+i*120)); }, []);

  const desktopOrder = [top3[1], top3[0], top3[2]];
  const mobileOrder  = [top3[0], top3[1], top3[2]];
    ...
    <div style={{ minHeight:"100vh", fontFamily:"'DM Sans','Segoe UI',sans-serif", color:"#1A202C", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"relative", zIndex:1 }}>
        <div style={{ padding: isMobile ? "24px 16px 0" : "36px 28px 0" }}>
          ...
          <div style={{ maxWidth:980, margin:"0 auto 40px", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
            <div style={{ display:"flex", alignItems:"center", gap: isMobile ? 12 : 18 }}>
              <img src="https://upload.wikimedia.org/wikipedia/en/8/8e/Brigade_Group.svg" alt="Brigade Group" style={{ height: isMobile ? 44 : 60, width:"auto" }}/>
              <div style={{ borderLeft:`3px solid ${BRIGADE_BLUE}`, paddingLeft: isMobile ? 12 : 18 }}>
                <div style={{ fontSize:10, fontWeight:700, letterSpacing:3, color:BRIGADE_BLUE, textTransform:"uppercase", marginBottom:3 }}>Q1 2025 · Project Performance</div>
                <div style={{ fontSize: isMobile ? 22 : 32, fontWeight:800, color:"#0D1F35", letterSpacing:-0.5, lineHeight:1 }}>PropeLL Leaderboard</div>
              </div>
            </div>
            <div style={{ background:"#EBF4FF", color:BRIGADE_BLUE, border:`1px solid #BEE3F8`, borderRadius:8, padding:"8px 18px", fontSize:11, fontWeight:700 }}>As of 1-Mar-25</div>
          </div>
          ...
          {/* Table — single unified component for both mobile and desktop */}
            ...
            <div className="table-scroll" style={{ borderRadius:14, border:"1px solid #E2E8F0", boxShadow:"0 2px 12px rgba(0,0,0,0.05)" }}>
              <div style={{ background:"#FFFFFF", borderRadius:14 }}>
                ...
                <div style={{
                  ...
                  gap: isMobile ? 5 : 8,
                  background:"#F0F4FA", borderBottom:"1px solid #E2E8F0",
                  fontSize: isMobile ? 8 : 9, letterSpacing:1.5, color:"#718096", fontWeight:700, textTransform:"uppercase", alignItems:"end"
                }}>
                  {headers.map((h, i) => {
                    const isScore = isMobile ? (i >= 2 && i <= 7) : (i >= 2 && i <= 7);
                    return (
                      <div key={h} style={{ textAlign: isScore ? "center" : "left", lineHeight:1.4, wordBreak:"break-word", whiteSpace:"normal" }}>{h}</div>
                    );
                  })}
                </div>
                ...
                {/* Rows */}
                {rest.map((p, i) => (
                  <div key={p.name} className="table-row" style={{
                    display:"grid", gridTemplateColumns: grid,
                    ...
                    <span style={{ fontSize: isMobile ? 11 : 15, fontWeight:800, color:"#CBD5E0", textAlign:"center" }}>{p.rank}</span>
                    <span style={{ fontWeight:700, fontSize: isMobile ? 11 : 14, color:"#2D3748", wordBreak:"break-word" }}>{p.name}</span>
                    ...
                    {/* Rationale — always present, fixed 160px on mobile so it doesn't force row height */}
                    <span style={{
                      fontSize: isMobile ? 10 : 12,
                      color:"#718096", lineHeight:1.5,
                      display:"block",
                      wordBreak:"break-word", overflowWrap:"break-word",
                      // On mobile: clamp to 3 lines max to keep rows compact
                      ...(isMobile ? {
                        display:"-webkit-box",
                        WebkitLineClamp:3,
          ...
          <div style={{ maxWidth:980, margin:"24px auto 0", padding:"18px 22px", background:"#FFFFFF", borderRadius:10, border:"1px solid #E2E8F0", fontSize:12, color:"#718096", lineHeight:1.8 }}>
            <p style={{ margin:0 }}>This dashboard is intended to provide visibility into current standings. The final scores will be consolidated and frozen at the end of each quarter.</p>
            <br/>
            <p style={{ margin:0 }}>For detailed scoring criteria, please refer to{" "}
              <a href="https://tiny.cc/propellkm" target="_blank" rel="noopener noreferrer" style={{ color:BRIGADE_BLUE, fontWeight:600, textDecoration:"underline", textUnderlineOffset:2 }}>this document</a>.
            </p>
            <br/>
            <p style={{ margin:0 }}>In case of any questions, please reach out to your department's PropeLL champion.</p>
          </div>
          <div style={{ height:32 }}/>
        </div>
        ...
        <div style={{ width:"100%", padding:"16px 28px", borderTop:"1px solid #E2E8F0", textAlign:"center", background:"#F4F7FB" }}>
          <div style={{ fontSize:12, color:"#A0AEC0", fontWeight:500 }}>Made with ♥ by the PropeLL team</div>
        </div>
      </div>
    </div>
  );
